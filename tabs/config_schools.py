...
def render(conn, df_ecoles, df_config, data_fwb):
    ...
    with col_l:
        ...
        if c_sel != "— Sélectionnez —":
            ...
            with st.expander(f"⚡ Actions groupées pour {c_sel}"):
                ...
                with ca2:
                    if st.button(f"Supprimer {c_sel} de la config", use_container_width=True, key="mass_del"):
    ...
    with col_r:
        # --- BLOCS STATS SYNCHRONISÉS (22px partout) ---
        st.markdown(f"""
...
<div style="text-align:center;"><b style="font-size:22px; color:#a78bfa; font-weight:900;">{active_communes.__len__()}</b><br><span style="font-size:22px;">Communes</span></div>
</div></div>
