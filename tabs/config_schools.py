...
def render(conn, df_ecoles, df_config, df_contacts, df_time, data_fwb):
    ...
    with col_l:
        ...
        if c_sel != "— Sélectionnez —":
            with st.expander(f"⚡ Actions groupées pour {c_sel}"):
                ...
                with ca1:
                    if st.button(f"Tout {c_sel} à 'NON'", use_container_width=True):
                        f_list = df_ecoles[df_ecoles['Commune'] == c_sel]['Fase école'].astype(str).unique()
                        ...
                        safe_write(conn, "EcolesConfig", df_upd); st.cache_data.clear(); st.rerun()
                with ca2:
                    if st.button(f"Réinitialiser {c_sel}", use_container_width=True):
            ...
            if e_fase:
                ...
                with st.form("form_cfg_indiv"):
                    f1, f2 = st.columns(2)
                    v_ex = f1.radio("Utilise l'Extrascolaire ?", ["Oui", "Non"], index=idx_ex, horizontal=True)
                    v_pa = f2.radio("Paiement", ["Prépaiement", "Post-paiement"], index=0 if (curr.empty or curr.iloc[0]['Paiement'] != "Post-paiement") else 1, horizontal=True)
    ...
    with col_r:
        ...
        st.download_button("🛡️ Sécurité des données : exporter toutes vos données vers Excel", buf_all.getvalue(), f"BACKUP_TOTAL_{datetime.now().strftime('%d_%m')}.xlsx", use_container_width=True)
    ...
    with col_list:
        ...
        if not df_target.empty:
            ...
            for i, (_, r) in enumerate(df_disp.iterrows()):
                ...
                r1.write(r['Commune'])
                r2.write(f"{r['Ecole']} ({r['Fase école']})")
                if not is_refus:
                    ...
                    for s in str(r['Services']).split('|'):
                        if s.strip() and s.strip() != "-":
                            html += f'<span style="background:{colors.get(s.strip(),"#999")}; color:white; padding:2px 6px; border-radius:4px; font-size:10px; margin-right:3px; display:inline-block; margin-top:2px;">{s.strip()}</span>'
...
<div style="font-size:26px; font-weight:bold;">{n_com} <small style="font-size:14px; font-weight:normal;">Communes</small></div>
<div style="font-size:26px; font-weight:bold;">{n_eco} <small style="font-size:14px; font-weight:normal;">Écoles</small></div></div>""", unsafe_allow_html=True)
        ...
        if not is_refus and not df_target.empty:
            ...
            if all_s:
                ...
                df_s.columns = ['S', 'N']
                fig_s = px.bar(df_s, x='N', y='S', orientation='h', height=300, text='N', color='S', color_discrete_map={"Cantine Jour": "#FFD700", "Cantine Semaine": "#FF8C00", "Cantine Mois": "#FF0000", "Garderie": "#38bdf8", "Activités": "#4ade80"})
        ...
        if not df_target.empty:
            ...
            with pd.ExcelWriter(buf_rep, engine='xlsxwriter') as wr:
                df_target.to_excel(wr, index=False)
            st.download_button("📥 Export Excel (Filtres)", buf_rep.getvalue(), "rapport_creos.xlsx", use_container_width=True)

            print_html = f"<html><head><style>body{{font-family:sans-serif;padding:20px;}}h1{{color:#008080;}}table{{width:100%;border-collapse:collapse;}}th,td{{border:1px solid #ddd;padding:8px;font-size:12px;}}th{{background:#008080;color:white;}}</style></head><body><h1>Rapport : {view_mode}</h1><table><thead><tr><th>Province</th><th>Commune</th><th>École</th><th>Paiement</th><th>Services</th></tr></thead><tbody>"
            ...
            st.markdown(f'<a href="data:text/html;base64,{b64}" target="_blank" style="text-decoration:none;"><div style="text-align:center;padding:10px;background:#1e293b;color:white;border-radius:5px;font-weight:bold;">🖨️ IMPRIMER LE RAPPORT</div></a>', unsafe_allow_html=True)
