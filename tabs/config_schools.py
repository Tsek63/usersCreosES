...
def render(conn, df_ecoles, df_config, data_fwb):
    ...
    if not df_active.empty:
        ...
        with g2:
            ...
            del_fase = st.selectbox("Choisir école à supprimer", df_active['Fase école'].tolist())
            if st.button("Supprimer l'école sélectionnée"):
                safe_write(conn, "EcolesConfig", df_config[df_config['Fase école'] != del_fase])
                st.cache_data.clear()
                st.rerun()
