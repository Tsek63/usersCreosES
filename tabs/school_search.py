...
<div style="font-size:48px; font-weight:bold; color:#4ade80;">{len(active_communes)}</div>
    ...
    with c2:
        ...
        commune = st.selectbox("🏘️ Commune", [""] + c_list, 
    ...
    with cr:
        st.write("")
        if st.button("🗑️ Effacer", key="reset_tab2"):
...
✉️ <a href="mailto:{ct['Email']}">{ct['Email']}</a>
        ...
        with st.expander("➕ Ajouter un nouveau contact"):
            with st.form("new_ct"):
                ...
                ntf = f2.text_input("Tel fixe")
                ngs = f2.text_input("GSM")
                nem = st.text_input("Email")
                if st.form_submit_button("💾 Enregistrer"):
                    new_row = pd.DataFrame([{"Province": prov, "Commune": commune, "Titre": nt, "Nom": nn, "Téléphone": ntf, "GSM": ngs, "Email": nem}])
