...
def render(conn, df_ecoles, df_config, data_fwb, df_contacts):
    # --- 0. NETTOYAGE DES DONNÉES (Supprime les "nan") ---
...
📞 {tel_l}<br>📱 {gsm_l}<br>✉️ {mai_l}
        ...
        # --- BLOC AJOUTER UN CONTACT (RÉTABLI ICI) ---
        with st.expander("➕ Ajouter un nouveau contact"):
            with st.form("new_ct_form_tab2"):
                st.write(f"Nouveau contact pour **{commune_tab3}**")
                f_add1, f_add2 = st.columns(2)
                with f_add1:
                    nt = st.text_input("Titre (M., Mme, ...)")
                    nn = st.text_input("Nom Prénom")
                    nf = st.text_input("Téléphone fixe")
                with f_add2:
                    ng = st.text_input("GSM")
                    ne = st.text_input("Email")
                
                if st.form_submit_button("💾 Enregistrer ce contact", use_container_width=True):
                    if nn.strip() == "":
                        st.error("Le nom est obligatoire.")
                    else:
                        new_row = pd.DataFrame([{
                            "Province": prov_tab3 if prov_tab3 != "Toutes" else df_ecoles[df_ecoles['Commune']==commune_tab3]['Province'].iloc[0], 
