...
def render(conn, df_ecoles, df_config, data_fwb, df_contacts):
    ...
    # --- 2. FILTRES (Tri alphabétique + Bouton Effacer) ---
    ...
    with c1:
        ...
        prov_tab3 = st.selectbox("🗺️ Province", ["Toutes"] + prov_list, key=f"p_sel_{st.session_state.t3_rc}")
    ...
    with c2:
        ...
        commune_tab3 = st.selectbox("🏘️ Commune", [""] + comm_list, key=f"c_sel_{st.session_state.t3_rc}",
                                    format_func=lambda x: f"{icon_po(x)} {x}" if x else "Sélectionnez...")
    
    with c3:
        search_ecole = st.text_input("🔍 Rechercher une école", placeholder="Nom, Fase...", key=f"s_sel_{st.session_state.t3_rc}")

    with c_reset:
        st.write("") 
        if st.button("🗑️ Effacer", use_container_width=True, key="btn_reset"):
            st.session_state.t3_rc += 1
            st.rerun()

    if commune_tab3:
        # --- 3. SECTION CONTACTS (Avec GSM et Modification complète) ---
        ...
        if not contacts_comm.empty:
            ...
            for i, (idx, ct) in enumerate(contacts_comm.iterrows()):
                with cols_c[i % 3]:
                    t_link = f'<a href="tel:{ct["Téléphone"]}" style="color:#7c3aed;">{ct["Téléphone"]}</a>' if ct["Téléphone"] != "-" else "-"
                    g_link = f'<a href="tel:{ct["GSM"]}" style="color:#7c3aed;">{ct["GSM"]}</a>' if ct["GSM"] != "-" else "-"
                    m_link = f'<a href="mailto:{ct["Email"]}" style="color:#7c3aed;">{ct["Email"]}</a>' if ct["Email"] != "-" else "-"
                    ...
                    if b_edit.button("✏️ Modifier", key=f"edit_btn_{idx}"):
                        st.session_state[f"edit_mode_{idx}"] = True
                    if b_del.button("🗑️ Supprimer", key=f"del_btn_{idx}"):
                        safe_write(conn, "Contacts", df_contacts.drop(idx))
                        st.cache_data.clear(); st.rerun()
                    
                    if st.session_state.get(f"edit_mode_{idx}"):
                        with st.form(f"form_modify_contact_{idx}"):
                            st.write(f"**Modifier {ct['Nom']}**")
                            new_titre = st.text_input("Titre", value=ct['Titre'])
                            new_nom = st.text_input("Nom Prénom", value=ct['Nom'])
                            new_tel = st.text_input("Téléphone fixe", value=ct['Téléphone'])
                            new_gsm = st.text_input("GSM", value=ct['GSM'])
                            new_email = st.text_input("Email", value=ct['Email'])
                            
                            c_val, c_can = st.columns(2)
                            if c_val.form_submit_button("✅ Valider"):
                                df_contacts.loc[idx, ['Titre', 'Nom', 'Téléphone', 'GSM', 'Email']] = [new_titre, new_nom, new_tel, new_gsm, new_email]
                                safe_write(conn, "Contacts", df_contacts)
                                del st.session_state[f"edit_mode_{idx}"]
                                st.cache_data.clear(); st.rerun()
                            if c_can.form_submit_button("❌ Annuler"):
                                del st.session_state[f"edit_mode_{idx}"]
                                st.rerun()

        with st.expander("➕ Ajouter un nouveau contact"):
            with st.form("form_add_contact"):
                f1, f2 = st.columns(2)
                at = f1.text_input("Titre")
                an = f1.text_input("Nom Prénom")
                atf = f2.text_input("Tel Fixe")
                ag = f2.text_input("GSM")
                ae = st.text_input("Email")
                if st.form_submit_button("💾 Enregistrer"):
                    new_ct = pd.DataFrame([{"Province": prov_tab3, "Commune": commune_tab3, "Titre": at, "Nom": an, "Téléphone": atf, "GSM": ag, "Email": ae}])
        ...
        for i in range(0, len(df_disp), 2):
            ...
            for j in range(2):
                if i + j < len(df_disp):
                    ...
                    e_m_link = f'<a href="mailto:{sch["Email"]}" style="color:#4169E1; text-decoration:none;">{sch["Email"]}</a>' if sch["Email"] != "-" else "-"
                    e_t_link = f'<a href="tel:{sch["Téléphone"]}" style="color:#1e293b; text-decoration:none;">{sch["Téléphone"]}</a>' if sch["Téléphone"] != "-" else "-"
                    ...
                    """
                    with cols[j]:
                        st.markdown(html_school_card, unsafe_allow_html=True)
