...
def render(df_ecoles, df_config, data_fwb):
    ...
    # Construction de df_tab1 (le résumé par commune pour la carte)
    tab1_rows = []
            ...
            ["Bruxelles", "Brabant Wallon", "Hainaut", "Liège", "Namur", "Luxembourg"].forEach(p => {{
                ...
                if(filtered.length > 0) {{
                    ...
                    filtered.forEach(x => {{
                        ...
                        const counts = `<div class="counts-container"><span class="cnt" style="background:#22c55e" title="Utilisent Creos">✓ ${{x.NbOui}} Écoles</span><span class="cnt" style="background:#ef4444" title="N'utilisent pas">✗ ${{x.NbNon}}</span><span class="cnt" style="background:#94a3b8" title="Inconnu">? ${{x.NbSans}}</span></div>`;
