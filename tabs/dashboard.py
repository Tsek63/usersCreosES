...
def render(df_ecoles, df_config, data_fwb, df_contacts):
        ...
        function renderList() {{
            ...
            provinces.forEach(p => {{
                ...
                if(filtered.length > 0) {{
                    ...
                    let displayProvName = (p === "Bruxelles") ? "BRUXELLES RÉGION-CAPITALE" : p.toUpperCase();
                    h.innerText = displayProvName; listDiv.appendChild(h);
                    filtered.forEach(x => {{
    ...
    with aud1: audit_card("Communes sans contact spécifique", f"{len(sans_contact)} commune(s)", "#ef4444" if sans_contact else "#22c55e", "⚠️")
