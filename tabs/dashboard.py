...
def render(df_ecoles, df_config, data_fwb):
    # --- 1. PRÉPARATION DES DONNÉES (Identique à votre original) ---
    ...
    <style>
        ...
        .cnt {{ padding: 2px 8px; border-radius: 4px; color: white; font-size: 11px; font-weight: bold; white-space: nowrap; }}
    </style></head>
    <body onload="init()">
    <div id="left">
        <div id="map-box"><svg id="svg" viewBox="0 0 900 650"></svg></div>
        <div class="stats-panel">
            <div class="panel-header"><div style="font-size:11px; opacity:0.7;">COMMUNES ACTIVES</div><div style="font-size:36px; font-weight:bold;">{t_dash}</div></div>
            <div style="display:flex; gap:15px;">
                <div style="flex:1;">
                    <div style="font-size:10px; opacity:0.5; text-align:center; margin-bottom:5px;">PAIEMENT</div>
                    <div style="display:flex; justify-content:space-between; margin-bottom:4px;"><span>Prépaiement</span><span style="font-weight:bold; color:#ec4899;">{p_dash}</span></div>
                    <div style="display:flex; justify-content:space-between;"><span>Post-paiement</span><span style="font-weight:bold; color:#38bdf8;">{po_dash}</span></div>
                </div>
                <div style="flex:1;">
                    <div style="font-size:10px; opacity:0.5; text-align:center; margin-bottom:5px;">SERVICES</div>
                    {"".join([f'<div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:2px;"><span>{k}</span><span style="font-weight:bold; background:{v[1]}; padding:0 6px; border-radius:4px;">{v[0]}</span></div>' for k,v in s_dash.items()])}
                </div>
            </div>
        </div>
    </div>
        ...
        function init() {{
            ...
            Object.entries(mapRef).forEach(([provName, list]) => {{
                ...
                list.forEach((name, i) => {{
                    ...
                    // TOOLTIP (Nom de la commune au survol)
        ...
        function renderList() {{
            ...
            provinces.forEach(p => {{
                ...
                if(filtered.length > 0) {{
                    ...
                    h.style.background='#f8fafc'; h.style.padding='6px 10px'; h.style.fontSize='11px'; h.style.fontWeight='bold'; h.style.color='#64748b'; h.style.marginTop='10px';
                    ...
                    filtered.forEach(x => {{
                        ...
                        row.innerHTML = `<span><strong style="color:#4169E1;">${{x.Commune}}</strong></span>
                            <div class="counts-container">
                                <span class="cnt" style="background:#22c55e" title="Utilisent Creos">✓ ${{x.NbOui}}</span>
                                <span class="cnt" style="background:#ef4444" title="N'utilisent pas Creos">✗ ${{x.NbNon}}</span>
