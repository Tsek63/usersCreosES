...
# 3. DONNÉES FWB (Les 262 communes)
data_fwb = {
...
# 4. TABS
tab1, tab2 = st.tabs(["📊 Dashboard", "✏️ Gestion"])

with tab1:
    st.title("Tableau de Bord")
    st.metric("Total Communes", len(df_gsheets))
    # Ici se trouvait votre carte HTML - je la simplifie pour garantir l'affichage
    st.dataframe(df_gsheets)

with tab2:
    st.header("Gestion des données")
    # Formulaire simplifié pour restaurer la fonction de base
    prov = st.selectbox("Province", list(data_fwb.keys()))
    comm = st.selectbox("Commune", data_fwb[prov])
    if st.button("Enregistrer"):
        st.success(f"{comm} enregistré (Simulation)")
