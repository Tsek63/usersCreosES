...
# CSS : Correction majeure pour ne rien masquer à l'écran
st.markdown("""
    <style>
        ...
        @media print {
            /* On cache tout ce qui est inutile sur papier */
            .no-print, .stSidebar, button, footer, header, .stTabs [data-baseweb="tab-list"] { 
                display: none !important; 
            }
            /* On force l'affichage de la zone d'impression */
            .print-only { 
        ...
        /* Masqué par défaut à l'écran */
        .print-only { display: none; }
        
        /* Style du tableau d'impression */
        ...
        .print-province-title { background-color: #f0f0f0; font-weight: bold; padding: 8px; border: 1px solid #333; }
    </style>
...
with tab1:
    ...
    html_map = f"""<!DOCTYPE html><html><head><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><style>
            ...
            .item-row {{ display: flex; justify-content: space-between; padding: 8px; border-bottom: 1px solid #f1f5f9; font-size: 12px; }}
        </style></head><body onload="init()">
    <div id="left">
        <div id="map-box"><svg id="svg" viewBox="0 0 900 650" style="width:100%; height:100%;"></svg></div>
        <div class="stats-panel">
            <div style="text-align:center;"><div style="font-size:30px; font-weight:bold;">{t_dash}</div><div style="font-size:10px; opacity:0.7;">COMMUNES</div></div>
    ...
    <div id="right"><input type="text" id="search" placeholder="🔍 Rechercher..." style="width:95%; padding:8px; margin-bottom:8px;" onkeyup="doSearch()"><div id="list"></div></div>
    <script>
...
with tab2:
    ...
    with col_s:
        ...
        if not df_gsheets.empty:
            fig_p = px.pie(df_gsheets, names='Paiement', hole=0.4, height=220, color_discrete_map={'Prépaiement':'#ec4899', 'Post-paiement':'#38bdf8'})
            fig_p.update_layout(showlegend=False, margin=dict(l=0,r=0,t=0,b=0))
            st.plotly_chart(fig_p, use_container_width=True)

    st.divider()

    # --- FILTRES ET IMPRESSION ---
    col_filt, col_btns = st.columns([7, 3])
    with col_btns:
        st.write("##")
    ...
    f1, f2 = st.columns(2)
    with f1: fl_p = st.multiselect("Province", sorted(df_gsheets['Province'].unique()))
    with f2: fl_m = st.multiselect("Paiement", ["Prépaiement", "Post-paiement"])
    ...
    print_html = f"""<div class="print-only">
        <h2 style='text-align:center;'>Liste des Utilisateurs Creos Extrascolaire</h2>
        <p style='text-align:center;'>Nombre total de communes : {len(df_res)}</p>
    """
    for prov in sorted(df_res['Province'].unique()):
        print_html += f"<div class='print-province-title'>{prov}</div><table class='print-table'><tr><th>Commune</th><th>Mode de Paiement</th><th>Services</th></tr>"
