...
# Sécurité : Vérification des colonnes pour éviter le KeyError
    ...
    with col_f:
        ...
        with st.form("edit_form"):
            ...
            with c2:
                ...
                serv_v = st.multiselect("4. Services", list(service_colors.keys()))
...
with tab_dash:
    ...
    with f3: fl_s = st.multiselect("Services", list(service_colors.keys()), key=f"s_{st.session_state.rc}")
    with f4:
    ...
    with c_v:
        if not df_sorted.empty:
            ...
            all_s = list(service_colors.keys())
            counts = [df_sorted['Services'].str.contains(s, na=False).sum() for s in all_s]
            fig_s = px.bar(x=all_s, y=counts, color=all_s, title="Popularité des Services",
                           color_discrete_map=service_colors)
            fig_s.update_layout(showlegend=False, xaxis_title="", yaxis_title="Nombre")
